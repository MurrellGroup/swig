...
async function initialize(message){
  ...
  if(message.tuning){
    ...
    const accepted=runtime.swig_set_tuning_options(
      tuning.dMatch,tuning.dMismatch,tuning.dGapOpen,tuning.dGapExtend,tuning.topD,tuning.minDMatch,
      tuning.jMatch,tuning.jMismatch,tuning.jGapOpen,tuning.jGapExtend,tuning.topJ,tuning.minJLength,
    );
    if(accepted!==0)throw new Error("SwiftIG rejected the requested D/J compatibility controls.");
  }
  const allocations=[message.references.V,message.references.D,message.references.J,message.references.C].map((value)=>put(encoder.encode(value||"")));
  try{
...
function annotate(message){
  ...
  try{
    ...
    count=option&&option.mode!=="off"
      ? runtime.swig_annotate_double_d(pointer,size,message.format,Math.round(message.minimumIdentity*1000),message.strand,option.mode==="all"?1:2,Math.round(option.minimumVjSpan),Math.round(option.seedLength),Math.round(option.pseudoTrim),Math.round(option.maximumPseudoMismatches),Math.round(option.minimumScoreGain))
