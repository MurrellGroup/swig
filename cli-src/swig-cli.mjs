...
function inputLines(path){
  ...
  const stream=/\.gz$/i.test(path)?raw.pipe(createGunzip()):raw;
...
async function* sequenceBatches(input,batchRecords){
  ...
  if(format==="airr"){
    ...
    for await(const line of lines){
      if(!header){if(!line.trim())continue;header=line.replace(/\r$/,"");continue;}
      if(!line)continue;body.push(line);
      if(body.length>=batchRecords){yield {format,count:body.length,header,body:body.join("\n")+"\n"};body=[];}
    }
    if(!header.includes("\t"))throw new Error(`${input.path} is not an AIRR TSV.`);
    if(body.length)yield {format,count:body.length,header,body:body.join("\n")+"\n"};
    return;
  }
  if(format==="fastq"){
    let record=[];let batch=[];let count=0;
    for await(const line of lines){
      record.push(line);
      ...
      if(batch.length>=batchRecords){yield {format,count:batch.length,text:batch.join("\n")+"\n"};batch=[];}
    }
    if(record.length)throw new Error(`${input.path} ends inside a FASTQ record.`);
    if(batch.length)yield {format,count:batch.length,text:batch.join("\n")+"\n"};
    return;
  }
  let current=[];let batch=[];
  for await(const line of lines){
    if(line.startsWith(">")){
...
function runChimera(rows,activeMask,config,scope,headers){
  ...
  const options={method,priorProbability:config.priorProbability,baseMutationProbability:config.baseMutationProbability,mutationRates:config.mutationRates,mutationSwitchProbability:config.mutationSwitchProbability,detailed:false};
  ...
  for(let ordinal=0;ordinal<rows.length;ordinal+=1){
    ...
    try{
      ...
      if(probability===undefined){probability=runChmm(msa,threadSequenceToMsa(sequence,germline,call,msa),sequence,germline,options).probability;cache.set(key,probability);}
      row.swig_chimera_probability=String(probability);row.swig_chimera_status="evaluated";evaluated+=1;
      if(probability>=config.posteriorThreshold){activeMask[ordinal]=0;flagged+=1;}
...
async function runPipeline(config,base,assets){
  ...
  const needsAnnotation=config.inputs.some((input)=>detectFormat(input.path,input.format)!=="airr");
  ...
  const rows=[];const headers=[];let annotatedRecords=0;let inputRecords=0;
  try{
    for(const input of config.inputs){
      ...
      const consume=async(item)=>{
        ...
        addHeaders(headers,["swig_dataset_id","sample_id","subject_id","swig_cohort","swig_timepoint","swig_compartment"]);
        for(const row of table.rows){
          row.swig_dataset_id=input.datasetId;row.sample_id=input.sampleId;row.subject_id=input.subjectId;row.swig_cohort=input.cohort;row.swig_timepoint=input.timepoint;row.swig_compartment=input.compartment;
          const dd=ddById.get(row.sequence_id);if(dd){Object.assign(row,dd);addHeaders(headers,Object.keys(dd));}
          rows.push(row);
        }
        annotatedRecords+=table.rows.length;inputRecords+=item.count;
      };
      for await(const batch of sequenceBatches(input,config.annotation.batchRecords)){
        const promise=batch.format==="airr"
          ? Promise.resolve({direct:true,header:batch.header,body:batch.body,doubleDHeader:"",doubleDBody:""})
          : pool.run({text:batch.text,count:batch.count,format:batch.format==="fasta"?1:2,minimumIdentity:config.annotation.minimumIdentity,strand:config.annotation.strand,doubleD:config.annotation.doubleD});
...
export async function runCli(assets=defaultCliAssets()){
  ...
  config.inputs=config.inputs.map((input)=>({...input,path:resolveFrom(base,input.path)}));
