...
3. **Results** downloads the complete AIRR TSV, opens a repertoire dashboard, or explores the local result index. Runs of one to three records open directly into evidence; larger runs stay paged and filterable.

Selecting a record scrolls directly to its full evidence panel. That view exposes its rearrangement map, framework/CDR regions, junction evidence, every populated AIRR field, and separate query-to-germline V, D, J, and optional C alignments. Exact co-optimal calls are shown together; optimized v0.4 builds also retain sparse near-tied candidate evidence with score, identity, and coordinates when a candidate is within two score points or 1% of the selected score. Full alternate alignment strings are intentionally not duplicated into every AIRR row. The alignment viewer switches between nucleotide and amino-acid modes. Protein alignments are translated and calculated only for the selected record.
...
Swig runs SwiftIG, not IgBLAST. Receptor behavior is determined from the selected locus and locus-bearing gene identifiers rather than an IgBLAST `ig_seqtype` setting. Custom germline identifiers should contain one of the supported locus names above. The current bundled release contains V/D/J references; upload a matching C FASTA to call isotype immediately. The reference-pack builder now retains IMGT C-REGION records when they are present in a future source release.
