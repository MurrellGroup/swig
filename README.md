...
3. **Results** downloads the complete AIRR TSV, opens a repertoire dashboard, explores the local result index, or runs opt-in post-analysis. An opt-in double-D screen has a separate evidence export and does not rewrite that ordinary AIRR table. Runs of one to three records open directly at the record detail; larger runs remain paged and filterable.
...
- **Explicit cumulative working set:** deduplication and CHMMAIRRa exclusion are separate applyable stages. Before committing either filter, the interface shows its threshold/action and retained/excluded counts. CHMMAIRRa, lineage assignment, initial sequence queries, and single-linkage expansion consume the current working set. The authoritative mask is read from the post-analysis worker immediately before the CHMMAIRRa scan, and excluded rows are not decompressed or submitted to its HMM workers. Reset restores all assigned records; the underlying AIRR result is not deleted.
...
SwiftIG/Swig 0.12.0 is research software. Benchmark study-critical calls and post-analysis thresholds against an independently validated workflow for the organism, assay, read length, somatic-hypermutation regime, and germline set relevant to the analysis. The 85% CDR3 identity lineage threshold, method D's abundance ratio, and double-D evidence defaults are starting values, not universal biological constants. Public germline coverage is not available for every animal species; arbitrary or partial new sets can be supplied with the per-segment upload controls.
