...
type LineageSortKey = "id" | "studyGroup" | "abundance" | "uniqueMembers" | "sampleCount" | "locus" | "vCalls" | "jCalls" | "cdr3Length" | "shmMean" | "shmMaximum" | "shmP95";
...
export function PostAnalysisWorkbench({ store, references, scope, loci, inputName, workers, callingProfile, minimumIdentity, strand, datasets = [], sampleColors = {}, defaultCollapseScope = "sample", defaultLineageScope = "sample", autoPipeline, onInspect, sessionHandleRef, initialSession }: Props) {
      ...
      try {
        await runInActiveLock(async () => {
          ...
          if (autoPipeline.missingAlleles.enabled) {
            ...
            await store.scanAirrRows(fields, async (rows) => { for (const row of rows) accumulator.add(row.values, row.ordinal, assignments[row.ordinal] ?? 0); }, { batchSize: 1500, includeMask: activeMask ?? undefined, onProgress: (processed, total) => setProgress({ processed, total: total * 2 }) });
  ...
  useEffect(()=>{
    ...
    void (async()=>{
      ...
      try{
        ...
        const restoredLineages=rawRestoredLineages?{...rawRestoredLineages,summaries:rawRestoredLineages.summaries.map((summary)=>({...summary,studyScope:summary.studyScope??"global",studyGroup:summary.studyGroup||"complete study",sampleIds:summary.sampleIds??[],subjectIds:summary.subjectIds??[],timepoints:summary.timepoints??[],compartments:summary.compartments??[]}))}:null;
        ...
        if(restoredSelectedIds.length){
          ...
          if(summary){
            const limitPerLineage=restoredSelectedIds.length===1?500:Math.max(20,Math.floor(1000/restoredSelectedIds.length));const memberGroups=await runtime.lineageMembersMany(restoredSelectedIds,limitPerLineage);const rows=await store.detailMany(memberGroups.flatMap((group)=>group.ordinals));
  ...
  async function runMissingAlleleAnalysis() {
    ...
    const result=await operation("Two-pass screening for linked V germline discrepancies",async()=>{
      ...
      await store.scanAirrRows(fields,async(rows)=>{for(const row of rows)accumulator.add(row.values,row.ordinal,assignments[row.ordinal]??0);},{batchSize:1500,includeMask:mask??undefined,onProgress:(processed,total)=>setProgress({processed,total:total*2})});
      const validator=accumulator.prepareValidation(references.V);
  ...
  async function searchLineageNeighbours() {
    ...
    try {
      ...
      if (neighbourMethod === "germline" || neighbourMethod === "either") {
        ...
        const representativeRows = await store.detailMany([...screens.values()].map((entry) => entry.screen.representativeOrdinal));
        ...
        const candidateRows = await store.detailMany(memberGroups.flatMap((group) => group.ordinals));
  ...
  const sortedLineageSummaries=useMemo(()=>{
    ...
    return summaries;
  },[lineages,lineageSort,lineageSearch,lineageLocusFilter,lineageMinAbundance,lineageMinUnique,lineageMinSamples,lineageMinCdr3Length,lineageMaxCdr3Length,lineageSampleFilterMode,lineageSelectedSamples,lineageMinimumShm,lineageShmFilterStatistic,lineageShmById]);
  ...
  return <section className="post-analysis-shell">
    ...
    <section className={moduleClass("lineage","post-module lineage-module")}>
      ...
      {lineages&&<section className="lineage-explorer-controls"><header><div><span className="section-kicker">Lineage explorer</span><strong>{sortedLineageSummaries.length.toLocaleString()} of {lineages.summaries.length.toLocaleString()} indexed summaries shown</strong><small>Filters compose with logical AND. Up to the 10,000 largest lineage summaries are retained for interactive interrogation.</small></div><button type="button" onClick={()=>{setLineageSearch("");setLineageLocusFilter("");setLineageSampleFilterMode("any");setLineageSelectedSamples(new Set());setLineageMinAbundance(0);setLineageMinUnique(0);setLineageMinSamples(0);setLineageMinCdr3Length(0);setLineageMaxCdr3Length(0);setLineageMinimumShm(0);}}>Clear lineage filters</button></header><div className="lineage-filter-grid"><label><span>Lineage, V, J, locus, or study group</span><CommitTextInput type="search" value={lineageSearch} onCommit={setLineageSearch} placeholder="contains…"/></label><label><span>Locus</span><select value={lineageLocusFilter} onChange={(event)=>setLineageLocusFilter(event.target.value)}><option value="">Any locus</option>{lineageLocusOptions.map((locus)=><option value={locus} key={locus}>{locus}</option>)}</select></label><label><span>Sample membership</span><select value={lineageSampleFilterMode} onChange={(event)=>setLineageSampleFilterMode(event.target.value as LineageSampleFilterMode)}><option value="any">Any sample pattern</option><option value="multiple">Multiple samples</option><option value="single">One sample only</option><option value="selected-any">Any selected sample</option><option value="selected-all">Every selected sample</option><option value="selected-only">No samples outside selection</option></select></label><label><span>Minimum abundance</span><CommitNumberInput min="0" value={lineageMinAbundance} blankWhenZero placeholder="Any" onCommit={setLineageMinAbundance}/></label><label><span>Minimum unique sequences</span><CommitNumberInput min="0" value={lineageMinUnique} blankWhenZero placeholder="Any" onCommit={setLineageMinUnique}/></label><label><span>Minimum sample breadth</span><CommitNumberInput min="0" value={lineageMinSamples} blankWhenZero placeholder="Any" onCommit={setLineageMinSamples}/></label><label><span>CDR3 nt minimum length</span><CommitNumberInput min="0" value={lineageMinCdr3Length} blankWhenZero placeholder="Any" onCommit={setLineageMinCdr3Length}/></label><label><span>CDR3 nt maximum length</span><CommitNumberInput min="0" value={lineageMaxCdr3Length} blankWhenZero placeholder="Any" onCommit={setLineageMaxCdr3Length}/></label>{shmDashboard&&<><label><span>SHM filter statistic</span><select value={lineageShmFilterStatistic} onChange={(event)=>setLineageShmFilterStatistic(event.target.value as LineageShmFilterStatistic)}><option value="mean">Weighted mean</option><option value="maximum">Maximum</option><option value="p95">Upper 95% quantile</option></select></label><label><span>Minimum SHM value</span><CommitNumberInput min="0" step="0.001" value={lineageMinimumShm} blankWhenZero placeholder="Any" onCommit={setLineageMinimumShm}/><small>{shmMetric.toLowerCase().includes("rate")?"Enter a fraction; 0.05 = 5%.":"Mutation count"}</small></label></>}</div>{lineageSampleOptions.length>1&&<fieldset className="lineage-sample-picker"><legend>Specific samples</legend>{lineageSampleOptions.map((sample)=><label key={sample}><input type="checkbox" checked={lineageSelectedSamples.has(sample)} onChange={(event)=>setLineageSelectedSamples((current)=>{const next=new Set(current);if(event.target.checked)next.add(sample);else next.delete(sample);return next;})}/><i style={{background:sampleColor(sample,sampleColors)}}/><span>{sample}</span></label>)}</fieldset>}<div className="lineage-table-wrap"><table><thead><tr><th><button type="button" onClick={()=>sortLineages("id")}>Lineage{sortIndicator("id")}</button></th><th>Merged</th><th><button type="button" onClick={()=>sortLineages("studyGroup")}>{DATASET_SCOPE_LABELS[lineageScope]}{sortIndicator("studyGroup")}</button></th><th><button type="button" onClick={()=>sortLineages("abundance")}>Abundance{sortIndicator("abundance")}</button></th><th><button type="button" onClick={()=>sortLineages("uniqueMembers")}>Unique{sortIndicator("uniqueMembers")}</button></th><th><button type="button" onClick={()=>sortLineages("sampleCount")}>Samples{sortIndicator("sampleCount")}</button></th><th><button type="button" onClick={()=>sortLineages("locus")}>Locus{sortIndicator("locus")}</button></th><th><button type="button" onClick={()=>sortLineages("vCalls")}>V calls{sortIndicator("vCalls")}</button></th><th><button type="button" onClick={()=>sortLineages("jCalls")}>J calls{sortIndicator("jCalls")}</button></th><th><button type="button" onClick={()=>sortLineages("cdr3Length")}>CDR3 nt{sortIndicator("cdr3Length")}</button></th>{shmDashboard&&<><th><button type="button" onClick={()=>sortLineages("shmMean")}>SHM mean{sortIndicator("shmMean")}</button></th><th><button type="button" onClick={()=>sortLineages("shmMaximum")}>SHM max{sortIndicator("shmMaximum")}</button></th><th><button type="button" onClick={()=>sortLineages("shmP95")}>SHM q95{sortIndicator("shmP95")}</button></th></>}<th/></tr></thead><tbody>{sortedLineageSummaries.slice(0,500).map((summary)=><tr key={summary.id} className={selectedLineageIds.includes(summary.id)?"selected":""} onClick={()=>void openLineage(summary)}><td><strong>{summary.id}</strong></td><td>{mergedIdByOriginal.get(summary.id)||"—"}</td><td>{summary.studyGroup}</td><td>{summary.abundance.toLocaleString()}</td><td>{summary.uniqueMembers.toLocaleString()}</td><td><strong>{(summary.sampleIds??[]).length}</strong><small title={(summary.sampleIds??[]).join(", ")}>{(summary.sampleIds??[]).slice(0,3).join(" · ")}{(summary.sampleIds??[]).length>3?` +${summary.sampleIds.length-3}`:""}</small></td><td>{summary.locus}</td><td>{summary.vCalls.join(", ")}</td><td>{summary.jCalls.join(", ")}</td><td>{summary.cdr3Length} nt</td>{shmDashboard&&<><td>{lineageShmLabel(summary.id,"mean")}</td><td>{lineageShmLabel(summary.id,"maximum")}</td><td>{lineageShmLabel(summary.id,"p95")}</td></>}<td><button type="button">Open →</button></td></tr>)}</tbody></table>{!sortedLineageSummaries.length&&<div className="empty-results"><span>∅</span><h3>No indexed lineage matches these filters.</h3></div>}</div></section>}
    </section>

    <section className={moduleClass("diagnostics","post-module downstream-viz-module")}>
      <header><div className="module-number amber">05</div><div><span className="section-kicker">Selected-population diagnostics</span><h3>Somatic hypermutation and reference-set evidence</h3><p>Both analyses inherit the committed population above. SHM summaries retain collapsed abundance; missing-V screening tests linked nucleotide haplotypes across independent lineages and then checks every retained member of each lineage.</p></div><a href="https://pmc.ncbi.nlm.nih.gov/articles/PMC5187446/" target="_blank" rel="noreferrer">IgDiscover method ↗</a><button className="module-collapse-toggle" type="button" aria-expanded={openModules.has("diagnostics")} onClick={()=>toggleModule("diagnostics")}>{openModules.has("diagnostics")?"Collapse ↑":"Expand ↓"}</button></header>
      <div className="diagnostic-grid">
        <article><span className="section-kicker">SHM</span><h4>Mutation distributions</h4><div className="control-grid three"><label><span>Measure</span><select value={shmMetric} onChange={(event)=>{setShmMetric(event.target.value as ShmMetricKey);setShmDashboard(null);}}><option value="vNtRate">V nucleotide rate</option><option value="vNtMutations">V nucleotide count</option><option value="vAaRate">V amino-acid replacement rate</option><option value="vAaReplacements">V amino-acid replacement count</option><option value="synonymous">Synonymous codon count</option><option value="cdrNtRate">CDR1/2 nucleotide rate</option><option value="frameworkNtRate">Framework nucleotide rate</option></select></label><label><span>Stratify by</span><select value={shmStratum} onChange={(event)=>{setShmStratum(event.target.value as typeof shmStratum);setShmDashboard(null);}}><option value="all">No additional stratum</option><option value="sample_id">Sample</option><option value="subject_id">Donor / subject</option><option value="swig_cohort">Cohort</option><option value="swig_timepoint">Timepoint</option><option value="swig_compartment">Compartment / tissue</option><option value="locus">Locus</option><option value="v_call">V call</option><option value="isotype">Isotype</option></select></label><label><span>Plot sample / lineage</span><CommitNumberInput min="50" max="10000" step="50" value={shmSampleCap} onCommit={(value)=>{setShmSampleCap(value);setShmDashboard(null);}} /></label></div><button className={guidedClass("run-shm")} type="button" disabled={Boolean(busy)} onClick={()=>void runShmAnalysis()}>Calculate SHM on {workingCount.toLocaleString()} records</button><p>Plot memory is bounded per lineage; scalar counts still cover every selected row.</p></article>
        <article><span className="section-kicker">Reference warning</span><h4>Possible missing V alleles</h4>
          <div className="control-grid three">
            <label><span>Independent unit</span><strong>Assigned lineage</strong><small>Record-level evidence is not permitted.</small></label>
          ...
          <details className="post-advanced"><summary>Evidence thresholds</summary><div className="control-grid three">
    ...
    {selectedLineage && <section ref={workbenchRef} className={moduleClass("workbench","post-module lineage-workbench")} tabIndex={-1}>
      ...
      {lineageGermline&&<div className="lineage-germline-method"><div><span className="section-kicker">Lineage root construction</span><label><span>Germline / UCA method</span><select value={lineageGermlineMethod} onChange={(event)=>{const method=event.target.value as LineageGermlineMethod;setLineageGermlineMethod(method);clearAlignmentArtifacts();clearNeighbourResults(true);}}><option value="closest">Closest member by matched V + J identity · default</option><option value="consensus">Equal-weight member consensus · alternative</option></select></label><strong>{lineageGermlineMethod==="closest"?`Template member: ${lineageGermline.selectedSequenceId||`AIRR row ${(lineageGermline.selectedOrdinal??0)+1}`}`:"Equal-weight AIRR-anchored member voting"}</strong><p>{lineageGermlineMethod==="closest"?"Swig ranks loaded members by equal-weight identity across informative matched V and J columns, then by combined identity, coverage, and AIRR order. The selected member supplies the V(D)J germline template and trimming endpoints; its observed bases fill only unresolved N junction sites in the comparison UCA. The N-masked template remains the tree root.":"Each loaded representative contributes one equal vote at every AIRR-anchored V/D/J germline column. A germline base requires ≥80% agreement among informative members; unresolved junction bases stay N in the tree root, while a separate comparison UCA may fill them with ≥60% unweighted query consensus. Endpoints require support from ≥20% of loaded members."}</p></div><div><b>{lineageGermline.rowsUsed.toLocaleString()}</b><span>{lineageGermlineMethod==="closest"?"member used":"members voted"}</span></div><div><b>{lineageGermline.knownColumns.toLocaleString()}</b><span>reference-resolved columns</span></div><div><b>{lineageGermline.inferredColumns.toLocaleString()}</b><span>N sites filled in comparison UCA</span></div><div><b>{lineageGermlineMethod==="closest"&&lineageGermline.selectedVjIdentity!==undefined?`${(lineageGermline.selectedVjIdentity*100).toFixed(2)}%`:lineageGermline.conflictingColumns.toLocaleString()}</b><span>{lineageGermlineMethod==="closest"?"equal-weight V/J identity":"conflicting columns retained as N"}</span></div></div>}
