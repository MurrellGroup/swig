...
type LineageSortKey = "id" | "studyGroup" | "abundance" | "uniqueMembers" | "locus" | "vCalls" | "jCalls" | "cdr3Length";
...
export function PostAnalysisWorkbench({ store, references, scope, loci, inputName, workers, callingProfile, minimumIdentity, strand, datasets = [], sampleColors = {}, defaultCollapseScope = "sample", defaultLineageScope = "sample", autoPipeline, onInspect, sessionHandleRef, initialSession }: Props) {
      ...
      try {
        await runInActiveLock(async () => {
          ...
          if (autoPipeline.missingAlleles.enabled) {
            ...
            setBusy("Pipeline · validating every lineage member (pass 2 of 2)");
  ...
  useEffect(()=>{
    ...
    void (async()=>{
      ...
      try{
        ...
        if(initialSession.shm){setShmMetric(initialSession.shm.metric);setShmDashboard(initialSession.shm.dashboard);}if(initialSession.missingAlleles){setMissingAlleleOptions({...DEFAULT_MISSING_ALLELE_OPTIONS,...initialSession.missingAlleles.options,unit:"lineage"});setMissingAlleles(initialSession.missingAlleles.dashboard?.validationPasses===2?initialSession.missingAlleles.dashboard:null);}
  ...
  async function runLineages() {
    ...
    if (result) {
      ...
      clearNeighbourResults(true);
      clearAlignmentArtifacts();
      advanceModule("lineage","diagnostics");
    }
  }
  ...
  async function openQueryLineage(hit:QueryLineageHit){
    ...
    if(!summary){
      ...
      summary={id:hit.lineageId,representativeOrdinal:record.ordinal,uniqueMembers:members.total,abundance:members.total,locus:record.locus,vCalls:record.vCall.split(",").map((value)=>value.trim()).filter(Boolean),jCalls:record.jCall.split(",").map((value)=>value.trim()).filter(Boolean),cdr3Length:record.cdr3.length,studyScope:lineageScope,studyGroup:datasetScopeValue(record,lineageScope)};
    }
  ...
  const sortedLineageSummaries=useMemo(()=>{
    const summaries=[...(lineages?.summaries??[])];
    const text=(summary:LineageSummary,key:LineageSortKey)=>key==="vCalls"?summary.vCalls.join(", "):key==="jCalls"?summary.jCalls.join(", "):String(summary[key]);
    const numeric=new Set<LineageSortKey>(["id","abundance","uniqueMembers","cdr3Length"]);
    summaries.sort((left,right)=>{const comparison=numeric.has(lineageSort.key)?Number(left[lineageSort.key])-Number(right[lineageSort.key]):text(left,lineageSort.key).localeCompare(text(right,lineageSort.key),undefined,{numeric:true,sensitivity:"base"});return (lineageSort.direction==="asc"?comparison:-comparison)||right.abundance-left.abundance||left.id-right.id;});
    return summaries;
  ...
  return <section className="post-analysis-shell">
    ...
    <section className={moduleClass("diagnostics","post-module downstream-viz-module")}>
      ...
      {shmDashboard?<ShmResultsPanel dashboard={shmDashboard} name={baseName(inputName)} color={chartColor} sampleColors={sampleColors} stratum={shmStratum}/>:null}
