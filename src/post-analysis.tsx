...
export function PostAnalysisWorkbench({ store, references, scope, loci, inputName, workers, callingProfile, minimumIdentity, strand, datasets = [], sampleColors = {}, defaultCollapseScope = "sample", defaultLineageScope = "sample", autoPipeline, onInspect, sessionHandleRef, initialSession }: Props) {
  ...
  return <section className="post-analysis-shell">
    ...
    <section className={moduleClass("diagnostics","post-module downstream-viz-module")}>
      <header><div className="module-number amber">05</div><div><span className="section-kicker">Selected-population diagnostics</span><h3>Somatic hypermutation and reference-set evidence</h3><p>Both analyses inherit the committed population above. SHM summaries retain collapsed abundance; missing-allele screening uses independent lineages by default to avoid counting the same clonal mutation repeatedly.</p></div><a href="https://pmc.ncbi.nlm.nih.gov/articles/PMC5187446/" target="_blank" rel="noreferrer">IgDiscover method ↗</a><button className="module-collapse-toggle" type="button" aria-expanded={openModules.has("diagnostics")} onClick={()=>toggleModule("diagnostics")}>{openModules.has("diagnostics")?"Collapse ↑":"Expand ↓"}</button></header>
