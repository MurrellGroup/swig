...
export function PhyloUcaPanel({ alignment, lineageRows, lineageIds, lineageLabel, locus, references, inputName, frameOffset, isTcr, sampleColors, multiplicityByOrdinal, lineageByOrdinal, initialState, onStateChange }: Props) {
  ...
  const matchingInitial = initialState?.alignmentFingerprint === fingerprint && initialState.lineageIds.join(",") === lineageIds.join(",") ? initialState : null;
  ...
  const identityKey = `${lineageIds.join(",")}|${fingerprint}`;
  ...
  useEffect(() => {
    ...
    const restored = initialState?.alignmentFingerprint === fingerprint && initialState.lineageIds.join(",") === lineageIds.join(",") ? initialState : null;
  ...
  useEffect(() => {
    onStateChange?.({ lineageIds: [...lineageIds], alignmentFingerprint: fingerprint, options, result: result ?? undefined });
  }, [fingerprint, lineageIds, onStateChange, options, result]);
  ...
  async function run() {
    ...
    try {
      ...
      const inference = await runPhyloUca({
        ...
        observedAlignmentFasta: observedTree.alignmentFasta,
  ...
  return <section className="phylo-uca-panel">
    ...
    <div className="phylo-uca-run-row">
      <div><strong>{options.characterMode === "auto" ? "Automatic GTR4 / gap-aware GTR5" : options.characterMode === "nucleotide-gtr4" ? "Forced nucleotide GTR4" : "Forced gap-aware GTR5"}</strong><span>Broad V/J hypotheses · all {references.counts.D.toLocaleString()} active D records · up to {options.hmm.maximumDSegments} D segments · local placement marginalization {options.search.marginalizeLocally ? "on" : "off"}</span></div>
    ...
    <details className="post-advanced phylo-uca-advanced">
      ...
      <div className="phylo-uca-advanced-body">
        <fieldset><legend>Character model</legend><label title="Auto uses four states for an ungapped observed alignment and five states only when an observed gap is present."><span>Alignment character model</span><select value={options.characterMode} onChange={(event) => { setOptions((current) => ({ ...current, characterMode: event.target.value as PhyloUcaOptions["characterMode"] })); setResult(null); }}><option value="auto">Automatic · GTR4 unless gaps occur</option><option value="nucleotide-gtr4">Force nucleotide GTR4 · gaps missing</option><option value="gap-aware-gtr5">Force A/C/G/T/gap GTR5</option></select></label><label title="Stationary frequency of the explicit alignment-gap character. Ignored by GTR4."><span>Gap equilibrium frequency</span><CommitNumberInput min="0.0001" max="0.5" step="0.005" value={options.model.frequencies[4]} onCommit={(value) => { setOptions((current) => ({ ...current, model: { ...current.model, frequencies: [current.model.frequencies[0], current.model.frequencies[1], current.model.frequencies[2], current.model.frequencies[3], value] } })); setResult(null); }} /></label></fieldset>
    ...
    {result && <div ref={resultRef} className="phylo-uca-results">
      <div className="phylo-uca-result-head"><div><span className="section-kicker">Maximum joint path + marginal sites</span><h4>{result.mapVCall || "V?"} · {result.mapDCalls.length ? result.mapDCalls.join(" → ") : "no D"} · {result.mapJCall || "J?"}</h4><p>{result.characterModel === "nucleotide-gtr4" ? "Four-state nucleotide GTR" : "Five-state A/C/G/T/gap GTR"} · edge {result.bestPlacement.endpointA} ↔ {result.bestPlacement.endpointB} at {(result.bestPlacement.edgeFraction * 100).toFixed(1)}% · UCA branch {result.bestPlacement.ucaBranchLength.toFixed(5)} · effective local placements {result.effectivePlacementCount.toFixed(2)}</p></div><div className="result-actions"><button type="button" onClick={() => download(`>phylo_UCA_joint_MAP_aligned\n${result.mapAlignedSequence}\n>phylo_UCA_marginal_consensus_aligned\n${result.posteriorConsensusAligned}\n`, `${base}.fasta`)}>UCA FASTA ↓</button><button type="button" onClick={() => download(posteriorTsv(result), `${base}.posterior.tsv`)}>Posterior TSV ↓</button><button type="button" onClick={() => download(result.placedTreeNewick, `${base}.placed-tree.nwk`)}>Placed tree ↓</button><button type="button" onClick={() => download(JSON.stringify(result, null, 2), `${base}.json`, "application/json")}>Complete JSON ↓</button></div></div>
