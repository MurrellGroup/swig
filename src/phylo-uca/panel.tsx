...
export function PhyloUcaPanel({ alignment, lineageRows, lineageIds, lineageLabel, locus, references, inputName, frameOffset, isTcr, sampleColors, multiplicityByOrdinal, lineageByOrdinal, initialState, onStateChange }: Props) {
  ...
  const annotationTracks = useMemo(() => collapseAndOrderHmmAnnotationTracks(result?.hmmAnnotations?.[annotationMode] ?? []), [annotationMode, result]);
    ...
    {result && <div ref={resultRef} className="phylo-uca-results">
      ...
      <section className="phylo-uca-logo">
        ...
        {result.hmmAnnotations && <div className="phylo-uca-annotation-toolbar"><div className="mode-toggle"><button className={annotationMode === "viterbi" ? "active" : ""} type="button" onClick={() => setAnnotationMode("viterbi")}>Best path</button><button className={annotationMode === "marginalized" ? "active" : ""} type="button" onClick={() => setAnnotationMode("marginalized")}>Marginalized</button></div><div className="phylo-uca-track-legend" aria-label="Track background colors"><span className="v">V</span><span className="n">NT</span><span className="d">D</span><span className="j">J</span></div><p>{annotationMode === "viterbi"
          ...
          : `Forward–backward source occupancy summed over HMM paths and retained tree placements. Identical D alleles are combined across retained registers/routes; NT rows combine non-template and unresolved boundary mass into spatial junction slots. Rows follow weighted V → NT → D block → NT → J order. ${annotationTracks.length} display tracks; allele groups appear at ≥ ${(result.hmmAnnotations.minimumDisplayedWeight * 100).toFixed(1)}%.${result.hmmAnnotations.omittedMarginalTrackCount ? ` ${result.hmmAnnotations.omittedMarginalTrackCount} subthreshold source track${result.hmmAnnotations.omittedMarginalTrackCount === 1 ? " is" : "s are"} hidden before display aggregation.` : ""}`}</p></div>}
        <div className="phylo-uca-posterior-viewport" onScroll={(event) => setAnnotationScrollLeft(event.currentTarget.scrollLeft)}>
          {result.hmmAnnotations && <div className="phylo-uca-annotation-scroll" style={{ width: logoLeftInset + logoContentWidth }}><PhyloUcaHmmAnnotationTracks tracks={annotationTracks} columns={annotationColumns} leftInset={logoLeftInset} contentWidth={logoContentWidth} labelOffset={annotationScrollLeft} title={`${lineageLabel} ${annotationMode === "viterbi" ? "best-path" : "marginalized"} phylo-HMM source annotation`} /></div>}
