...
- assignment strategy, calling profile, AIRR preserve/reannotate policy, strand, V/D/J identity floor, optional covered C-prefix identity floor, worker count, and Double-D settings;
...
- output naming and ordinary AIRR compression (`none` or `gzip`).
...
`swig-cli --vdj` is a separate execution route for high-volume assignment. It does not call `annotateAirrBatch`, construct study manifests, parse results into post-analysis records, retain repertoire rows, or run allele pooling/collapse/chimera/selection/lineage/SHM/missing-allele code. The query parser supplies one bounded FASTA batch to the same worker pool, completed promises are consumed in submission order, and each AIRR body is written immediately after its single header. `-out`/`--out` is mandatory; `-out -` is an explicit streamed stdout destination. An output path ending in `.gz` is gzip-compressed while plain `.tsv` and stdout remain uncompressed. AIRR writers replace tab/CR/LF characters inside user-controlled identifier or quality values so every record retains the header's tab-delimited field count. `-query -` reads FASTA from stdin. `-outfmt` currently accepts only `19` because other IgBLAST renderers are not SwiftIG output formats.

The route accepts the common IgBLAST names `-query`, `-germline_db_V`, `-germline_db_D`, `-germline_db_J`, `-c_region_db`, `-out`, `-outfmt`, `-organism`, `-ig_seqtype`, `-domain_system`, `-strand`, `-num_threads`, `-custom_internal_data`, `-auxiliary_data`, and `-d_frame_data`. Source germline arguments are nucleotide FASTA (plain or gzip), not BLAST database prefixes. `--workers` has higher precedence than `-num_threads`. SwiftIG-specific `--batch-records`, `--minimum-identity`, `--minimum-c-prefix-identity`, `--assigner`, and `--calling-profile` remain available. Omitting `--assigner` selects RIAT-MP, consistently with a normalized pipeline config and Swig Web. `--calling-profile r_optimized` and `--calling-profile sensitive_d` both require `--assigner aer_robust`; the same pairing is validated in pipeline JSON. R-optimized uses the shared 4-nt D signal floor and an evidence-conditioned 12-to-10 joint D cost; Sensitive-D uses the same admission/search/scoring path with a fixed joint cost of 10. The CLI help reports this distinction. A supplied C database uses the same post-J-only, ≥90%-local-identity rule as Swig Web: ordinary local hits need ≥30 aligned bases and `c_support <= 1e-5`, while short gapless 5′-anchored hits use a database-size-adjusted `c_support <= 1e-4` gate. The optional 0..1 `--minimum-c-prefix-identity` / `annotation.minimumConstantPrefixIdentity` control examines the complete covered prefix and, on failure, clears only C; the general `--minimum-identity` option remains the V/D/J floor. A small set of D/J search controls maps to the exported SwiftIG tuning ABI; if those controls override an AER-R-only profile, its endpoint and exact profile-specific D-presence decisions remain active through the two-parameter decision setter. Unsupported BLAST scoring/search/report options fail rather than being silently ignored.
